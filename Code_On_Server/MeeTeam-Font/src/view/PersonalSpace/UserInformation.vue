      ...
      <el-main>
        ...
        <div>
          ...
          >
            ...
            <el-table-column prop="project_status" label="是否有组队需求" width="150"></el-table-column>
...
import { copyhead } from "@/api/file_load.js";
  ...
  methods: {
    editInfo() {
      ...
      GetUserInfor(params).then((res) => {
        let item = res.data[0];
        this.form.contact_info = item.contact_info;
        if (item.gender === "0")
          this.form.gender = "女";
        else
          this.form.gender = "男";
        this.form.institution = item.institution;
    ...
    handleDelete(row) {//删除操作
      ...
      deletecollect(param);
      //列表中删除
      this.tableData1.splice(row.index, 1) //删除下标为index的内容
      this.$message({
        type: 'success',
        message: '删除成功!',
      })
    ...
    handleDetail(row) {
      ...
      this.$router.push({ path: "/users/ProjectDetail", query: { p_id: project_id, id: this.username } });
    },
    submit() {
      let vm = this;
      let params = {
